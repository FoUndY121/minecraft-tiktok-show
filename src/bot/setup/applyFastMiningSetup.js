...
async function applyFastMiningSetup({ bot, rcon } = {}) {
	...
	const commands = [
		...
		`/effect give ${bot.username} minecraft:speed 999999 5 true`,
		`/give ${bot.username} minecraft:netherite_pickaxe{Enchantments:[{id:"minecraft:efficiency",lvl:10s}]} 1`,
		`/give ${bot.username} minecraft:netherite_shovel{Enchantments:[{id:"minecraft:efficiency",lvl:10s}]} 1`,
	]
	...
	for (const command of commands) {
		try {
			...
			console.log(
				'[BOT_SETUP] fast mining command warning:',
				err?.message || err
			)
		}
	}
	...
	try {
		...
		const shovel = bot.inventory
			?.items?.()
			.find(item => item.name === 'netherite_shovel')
		const pickaxe = bot.inventory
			?.items?.()
			.find(item => item.name === 'netherite_pickaxe')
